	...
	Context("with offline bundle file", func() {
		...
		BeforeEach(func() {
			h = testing.SetupOffline(ginkgoT, func(cfg *config.Config) {
				cfg.Directory.Path = testing.AssetAcmeEBBFilePath()
		...
		Context("when using the engine directly", func() {
			Context("with basic functionality", func() {
				...
				It("loads eds plugin and data", func() {
					...
					Eventually(func() (interface{}, error) {
						result, err := h.Runtime().Query(h.ContextWithTenant(), `x = dir.user("auth0|7cee8e85-e3cf-4b6f-84ec-fac89107e957")`, nil, true, true, true, "full")
						if err != nil {
							return nil, err
						}
				...
				It("runs queries with functions that have parameters", func() {
					result, err := h.Runtime().Query(h.ContextWithTenant(), `x = dir.identity("euang@acmecorp.com")`, nil, true, true, true, "full")
					Expect(err).ToNot(HaveOccurred())
			...
			Context("with detailed query capabilities", func() {
				It("runs queries and returns metric data when requested", func() {
					result, err := h.Runtime().Query(h.ContextWithTenant(), `x = dir.identity("euang@acmecorp.com")`, nil, false, true, false, "off")
					Expect(err).ToNot(HaveOccurred())
				...
				It("runs queries and doesn't return metric data if not requested", func() {
					result, err := h.Runtime().Query(h.ContextWithTenant(), `x = dir.identity("euang@acmecorp.com")`, nil, false, false, false, "off")
					Expect(err).ToNot(HaveOccurred())
				...
				It("runs queries and returns an explanation when requested", func() {
					result, err := h.Runtime().Query(h.ContextWithTenant(), `x = dir.identity("euang@acmecorp.com")`, nil, false, false, false, "full")
					Expect(err).ToNot(HaveOccurred())
				...
				It("runs queries and returns a pretty explanation when requested", func() {
					result, err := h.Runtime().Query(h.ContextWithTenant(), `x = dir.identity("euang@acmecorp.com")`, nil, true, false, false, "full")
					Expect(err).ToNot(HaveOccurred())
				...
				It("runs queries and returns metrics if instrumentation requested", func() {
					result, err := h.Runtime().Query(h.ContextWithTenant(), `x = dir.identity("euang@acmecorp.com")`, nil, false, false, true, "off")
					Expect(err).ToNot(HaveOccurred())
